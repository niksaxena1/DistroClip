...
5. Click the folder path below the file cards to open the release folder in Explorer. Use the contract icon beside the release heading to open its PDF.
...
An amber **COVER** badge appears when a root-level Easy Song Proof of Licensing PDF is present. Click the badge to open that license. Songwriter names then appear from its `By ...` block. Warnings are shown when a PDF is unreadable or a choice needs verification.
...
Settings and the crash log are stored under `%LocalAppData%\DistroClip`. No release, contract, or usage data is uploaded anywhere by the app.
